...
func Signed(method jwt.SigningMethod, signingKey any, headers map[string]any, mapClaims jwt.MapClaims) (string, *jwt.Token, error) { //nolint:lll // Do not want to multi-line.
	...
	case expErr == nil && exp.Before(now):
		return "", nil, fmt.Errorf("%w: expiration time is in the past", ErrInvalidClaimValue)
	case expErr == nil:
	...
	case iatErr == nil && iat.After(now):
		return "", nil, fmt.Errorf("%w: issued at time is in the future", ErrInvalidClaimValue)
	case iatErr == nil:
...
// SignedFromContext is Signed, but does not take arguments for headers or claims. Instead, it
// unwraps these from the provided context. See WrapContextClaims and WrapContextHeaders for more.
...
// SignedWithExpiration is Signed, but takes the expiration by argument.
//
// If an expires-at is in the given jwt.MapClaims, it is ignored. Only the given
// expiration is used. Consumers can omit issued-at (claims.MapKeyIssuedAt) and
// not-before (claims.MapKeyNotBefore) to use the defaults, or specify them if
// they need something other than time.Now.
func SignedWithExpiration( //nolint:revive // Number of arguments necessary.
	...
	if expiration <= 0 {
		return "", nil, fmt.Errorf("%w: expiration must be greater than 0", ErrInvalidParam)
	}

	claims.SetExpiresAt(mapClaims, time.Now().Add(expiration))

	return Signed(method, signingKey, headers, mapClaims)
}
